...
<?php if(isset($create_announcement)) {
    ...
    <?php if ($_SESSION['user-role'] == 'Admin' or $_SESSION['user-role'] == 'Coordinator') {?>
        <form method="post" action="/site_announcement" class="announcement-card border">
            <div class="topic-container-add grid v-center h-justify">
                ...
                <button id="publishbtn" class="btn confirm-btn h-center v-center" onclick="return siteAnnouncementInsertValidate(document.getElementById('heading_textarea').value,document.getElementById('content_textarea').value)">Publish
                </button>
            </div>
    ...
    <div class="card-edit-modal">
        <form method="post" action="/update_site_announcement" class="announcement-card border announcement-card-border">
            ...
            <div class="modal-btns flex h-center">
                <button type="submit" id="publishbtn" class="btn confirm-btn h-center v-center modal-publish-btn" onclick="return siteAnnouncementUpdateValidate(document.getElementById('heading_textarea_edit').value,document.getElementById('content_textarea_edit').value)">Publish
                </button>
...
<script type="text/javascript">
    ...
    //validation
    function siteAnnouncementInsertValidate(heading,content) {
        if (!heading || !content) {
    ...
    function siteAnnouncementUpdateValidate(heading,content) {
        if (!heading || !content) {
            const modal = document.getElementById("warn-modal");
            modal.hidden = false;
            return false;
    ...
    const siteWarn = document.getElementById('warn-modal');
    document.addEventListener('click', function (event) {
        if (event.target === siteWarn) {
            siteWarn.style.display = 'none';
        }
    });
</script>